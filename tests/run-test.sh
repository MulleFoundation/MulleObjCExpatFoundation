...
#        ./mulle-objc-runtime
#
   MULLE_OBJC_EXPAT_DEPENDENCIES_INCLUDE=/usr/local/bin

   lib="`ls -1 ../lib/libMulleStandaloneObjCExpatFoundation.dylib 2> /dev/null | tail -1`"
   MULLE_OBJC_EXPAT_DEPENDENCIES_INCLUDE="../include"

   if [ ! -x "${lib}" ]
   then
      lib="`ls -1 "../build/Products/Debug/libMulleStandaloneObjCExpatFoundation.dylib" | tail -1 2> /dev/null`"
      MULLE_OBJC_EXPAT_DEPENDENCIES_INCLUDE="../dependencies/include"
   fi

   MULLE_OBJC_EXPAT="${1:-${lib}}"
   [ -z $# ] || shift

   if [ -z "${MULLE_OBJC_EXPAT}" ]
   then
      echo "libMulleStandaloneObjCExpatFoundation.dylib can not be found" >&2
      exit 1
   fi

   MULLE_OBJC_EXPAT_INCLUDE="`dirname "${MULLE_OBJC_EXPAT}"`"

   if [ -d "${MULLE_OBJC_EXPAT_INCLUDE}/usr/local/include" ]
   then
      MULLE_OBJC_EXPAT_INCLUDE="${MULLE_OBJC_EXPAT_INCLUDE}/usr/local/include"
   else
      MULLE_OBJC_EXPAT_INCLUDE="${MULLE_OBJC_EXPAT_INCLUDE}/include"
   fi
fi
...
fail_test()
{
   ...
   $MULLE_CLANG -O0 -g -o "${a_out}.debug" \
      -fobjc-runtime=mulle \
      "-I${MULLE_OBJC_EXPAT_INCLUDE}" \
      "-I${MULLE_OBJC_EXPAT_DEPENDENCIES_INCLUDE}" \
      "${MULLE_OBJC_EXPAT}" \
      "${m_source}" > "$errput" 2>&1

   echo "MULLE_OBJC_EXPAT_AUTORELEASEPOOL_TRACE=15 \
   MULLE_OBJC_EXPAT_TEST_ALLOCATOR=1 \
...
then
   ...
   MULLE_OBJC_EXPAT="`absolute_path_if_relative "$MULLE_OBJC_EXPAT"`"
   MULLE_OBJC_EXPAT_INCLUDE="`absolute_path_if_relative "$MULLE_OBJC_EXPAT_INCLUDE"`"
   MULLE_OBJC_EXPAT_DEPENDENCIES_INCLUDE="`absolute_path_if_relative "$MULLE_OBJC_EXPAT_DEPENDENCIES_INCLUDE"`"

   # OS X
   DYLD_FALLBACK_LIBRARY_PATH="`dirname "${MULLE_OBJC_EXPAT}"`" ; export DYLD_FALLBACK_LIBRARY_PATH
   # Linux
   LD_LIBRARY_PATH="`dirname "${MULLE_OBJC_EXPAT}"`" ; export LD_LIBRARY_PATH
